...
class tensor(handle_tensor):
    ...
    def __getitem__(self, i):
        ...
        if isinstance(i, tensor): 
            eidx = i.idx
            i = tensor(ones_like(self.arr), self.idx, self.ud) * i 
            i.transpose(self.idx.translate(None, eidx) + eidx)
            self.transpose(i.idx)
            Dlst = [0]; pre=1
            for v in array(i.arr.shape)[-1:0:-1]:
                Dlst += [v * pre]
                v = pre
            idx_t = 0
            Dlst.reverse()
            for num,D in enumerate(Dlst):

                i = i + tensor(arange(i.arr.shape[num]) * D, i.idx[num], i.ud[num])
            return tensor(self.arr.flatten()[i.arr.flatten().astype(int)].reshape(i.arr.shape), i.idx, i.ud)
        #if isinstance(i, tensor): 
        #    self.transpose(i.idx)
        #    Dlst = [1]; pre=1
        #    for v in array(i.arr.shape)[-1:0:-1]:
        ...
        #    idx_t = 0
        #    Dlst.reverse()
        #    i2 = tensor(zeros(i.arr.shape), i.idx, i.ud)
        #    for num,D in enumerate(Dlst):

        #        i2 = i2 + tensor(arange(i.arr.shape[num]) * D, i.idx[num], i.ud[num])
        #    self.arr = self.arr.flatten()[i2.arr.flatten().astype(int)].reshape(i.arr.shape)
        #    return tensor(self.arr, self.idx, self.ud)
...
def t_abs(obj):
    return tensor(abs(obj.arr), obj.idx, obj.ud)

def t_angle(obj):
    return tensor(angle(obj.arr), obj.idx, obj.ud)
