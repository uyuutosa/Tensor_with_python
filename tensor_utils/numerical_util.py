...
def permutation_symbol_array(n):
    ...
    global lst
    global lst2 
    lst = [":%s" %n for x in range(n)]
    lst2 = [":" for x in range(n+1)]
    lst2[-1] = "::-1"
    line = "global ret; global lst; global lst2; global ret; ret =  mgrid[" + ",".join(lst) + "].transpose()[" + ",".join(lst2) + "]"
    exec(line)
...
def prime(N):
    ...
    a = arange(2, N+1)
    for i in range(2, N+1): a = a[where((a <= i) + (a%i != 0))]
    return a
